...
@section('content')
    <div class="p-6">
        <div class="bg-white shadow-xl rounded-2xl p-8 w-full md:w-2/3 lg:w-1/2 mx-auto border border-gray-200">
            ...
            <form action="{{ route('users.update', request('tipe') == 'admin' ? $user->id_admin : $user->id_petugas) }}"
                ...
                @if (request('tipe') == 'admin')
                    ...
                    <div>
                        ...
                        <select name="role" id="role"
                            ...
                            <option value="padang" {{ old('role', $user->role) == 'padang' ? 'selected' : '' }}>Padang
                            </option>
                            <option value="solok" {{ old('role', $user->role) == 'solok' ? 'selected' : '' }}>Solok
                            </option>
                            <option value="sawah_lunto" {{ old('role', $user->role) == 'sawah_lunto' ? 'selected' : '' }}>
                                Sawah Lunto</option>
                        </select>
                        @error('role')
                            <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                ...
                <div>
                    <label for="password" class="block mb-1 text-sm font-medium text-gray-700">Password <span
                            class="text-gray-500 text-sm">(kosongkan jika tidak diganti)</span></label>
